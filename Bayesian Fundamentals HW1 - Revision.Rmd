...
No. The question of interest is to study the probability with respect to random variable Y, and we are interested in finding $Pr(Y>250)$. Since $Y \sim Bino(n, \theta)$,
$$Pr(Y > 250) = 1 - \sum_{i = 1}^{250}\binom{n}{i}\theta^i(1-\theta)^{250-i}$$
...
post_mean <- (theta/tau^2 + sum(huron$level)/(1.5^2))/(1/tau^2 + length(huron$level)/(1.5^2))
post_sd <- 1/(1/tau^2 + length(huron$level)/(1.5^2))
